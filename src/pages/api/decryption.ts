...
export default function handler(
  ...
  const decryptProcess = (Text: string[], key: number[]) => {
    return new Promise<string[]>((resolve) => {
      ...
      Text.forEach((t) => {
        ...
        if (needToSkip) {
          decryptedText.push(skipElememts[skipElememts.indexOf(t)]);
          return;
        }
        ...
        decryptedText.push(
          isCapital
            ? capitalLetters[tempIndex - key[0]]
            : smallLetters[tempIndex - key[0]]
        );
        // for infinite nested loop
        key.push(key.shift() as number);
      });
  ...
  const mainProcess = async () => {
    ...
    console.log({
      normalText: decryptedText,
      encryptedText: decryptedText,
      error: undefined,
    });
    res
      .status(200)
      .json({ normalText: decryptedText, encryptedText, error: undefined });
  };
  mainProcess();
  console.log("test");
}
